...
def load_data(name, root="./data", download=True, save_pre_data=True):
    ...
    if name == "MNIST":
        ...
        trainset = torchvision.datasets.ImageFolder(
            root="./data/tiny-imagenet-200/train", transform=train_val_transform
        )
        testset = torchvision.datasets.ImageFolder(
            root="./data/tiny-imagenet-200/val", transform=test_transform
        )
        trainset.targets = torch.Tensor(trainset.targets)
        testset.targets = torch.Tensor(testset.targets)
...
def divide_data(
    ...
    trainset, testset, len_classes = load_data(
